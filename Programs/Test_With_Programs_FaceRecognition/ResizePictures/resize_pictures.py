...
ap.add_argument("-f", "--format", default=".png", help="Format of the picture. .png .jpg")
...
for (i, imagePath) in enumerate(imagePaths):
	...
	print("Size before - H: {} - W: {}".format(img_H, img_W))
	...
	print("Size after - H: {} - W: {}".format(img_new.shape[0], img_new.shape[1]))
	
	#Store the resized image with the old name but in the folder precise in parameter
	if(args["change"] == "1"):
		path = path + imagePath[imagePath.find('/')+1:imagePath.find('.')] + args["format"]
	else:
		path += imagePath[imagePath.find('/')+1:]
	print(path)
